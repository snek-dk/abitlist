                ...
                for teg in soup.findAll('tr'):
                    ...
                    if len(teg) == 27:
                        ...
                        json_dict.append({
                            ...
                            'СНИЛС_УК': data[1],
                            'Конкурс': data[3],
                            'СУММА': str(data[4]).strip() if len(str(data[4]).strip()) > 0 else "0",
                            'СУММА_БЕЗ_ИД': str(data[5]).strip() if len(str(data[5]).strip()) > 0 else "0",
                            'ВИ_1': str(data[6]).strip() if len(str(data[6]).strip()) > 0 else "0",
                            'ВИ_2': str(data[7]).strip() if len(str(data[7]).strip()) > 0 else "0",
                            'ВИ_3': str(data[8]).strip() if len(str(data[8]).strip()) > 0 else "0",
                            'ВИ_4': None,
                            'ВИ_5': None,
                            'ИД': str(data[9]).strip() if len(str(data[9]).strip()) > 0 else "0",
                            'Согласие': data[12],
                            'Оригинал': data[11]
                        })
                        ...
                        if fak_num not in ("27.03.04", "42.03.01", "45.03.02"):
                            ...
                            if order_score == 'рми':
                                s1, s2, s3 = [json_dict[k]['ВИ_' + str(j)] for j in range(1,4)]
                                json_dict[k]['ВИ_1'], json_dict[k]['ВИ_2'], json_dict[k]['ВИ_3'] = s2, s3, s1

                            elif order_score == 'мри':
                                s1, s2, s3 = [json_dict[k]['ВИ_' + str(j)] for j in range(1,4)]
                                json_dict[k]['ВИ_1'], json_dict[k]['ВИ_2'], json_dict[k]['ВИ_3'] = s1, s3, s2
